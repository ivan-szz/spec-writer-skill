...
| Rule ID | Rule | Check |
|---------|------|-------|
| **TEST-001** | Every AC has ≥1 test | For each `AC-N` in Acceptance Criteria, the Test Strategy table must contain at least one row with `Acceptance Criterion` = `AC-N`. |
| **TEST-002** | Test level specified | Every row in the Test Strategy table must have a **Level** column set to exactly one of: `unit`, `integration`, `e2e`. No blanks. |
...
[PASS] TEST-001 — All 4 acceptance criteria map to ≥1 test
[PASS] TEST-002 — All 6 tests specify a level

Result: 9/9 PASS — spec is valid
```
...
| **ANT-004** | Orphan requirements | REQ row exists but no AC, test, or implementation note references it. | Link to at least one AC or mark as deferred with rationale. |
